...
- [x] `skills install` verb — install `SKILL.md` into agent-specific dirs (`.claude/skills/`, `.codex/skills/`, `.cursor/skills/`, etc, 16 total + common `.agents/skills/`), each as `<target>/frappe-ctl/SKILL.md` per the Agent Skills standard (corrected — ADR-028, original flat-file format from ADR-021 wasn't actually auto-discovered by any standard-compliant agent). Chctl-inspired. Non-interactive, `--detected-only` default. Turns the skill file from a doc into a real distribution mechanism. Unblocks the skills.sh push.
...
- [x] Push to skills.sh — turned out to have no submission/approval step at all: `npx skills add <owner>/<repo>` pulls directly from any public GitHub repo. Real blocker was a format mismatch — their discovery requires a file literally named `SKILL.md` with YAML frontmatter, which the previously-shipped `frappe-ctl.skill.md` (no frontmatter, deliberately named per ADR-021) didn't satisfy. First pass added a second duplicate file kept in lockstep via a test; caught on review that duplication was unnecessary — collapsed to a single canonical `SKILL.md`. See ADR-027. **Further correction (ADR-028):** the installed-copy format itself was also wrong — flat `frappe-ctl.skill.md`, no frontmatter, isn't discovered by any Agent-Skills-standard-compliant tool (verified against Claude Code's docs and skills.sh's own installer output). Fixed to `<target>/frappe-ctl/SKILL.md`, frontmatter kept. `npx skills add MalharDotTech/frappe-ctl` now works and installs into every agent correctly; badge + install instructions added to README.
