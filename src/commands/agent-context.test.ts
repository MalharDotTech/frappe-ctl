...
describe("cmdAgentContext", () => {
  ...
  it("lists all apps with alias and modules", async () => {
    const parsed = await getContext() as { apps: { alias: string; modules: string[] }[] };
  ...
  it("lists all 16 verbs with description and flags", async () => {
    const parsed = await getContext() as { verbs: { name: string; description: string }[] };
  ...
  it("workflow verb is not readonly_safe", async () => {
    const parsed = await getContext() as { verbs: { name: string; readonly_safe: boolean }[] };
  ...
  it("bulk verb documents sub-verbs and --force requirement", async () => {
    const parsed = await getContext() as { verbs: { name: string; flags: string[] }[] };
  ...
  it("includes examples for each verb", async () => {
    const parsed = await getContext() as { verbs: { name: string; example: string }[] };
  ...
  it("auth field documents both self-hosted and OAuth paths", async () => {
    const parsed = await getContext() as { auth: { self_hosted: string; frappe_cloud: string } };
  ...
  it("env_vars includes all three documented vars", async () => {
    const parsed = await getContext() as { env_vars: Record<string, string> };
  ...
  it("auth_commands lists login/logout/status", async () => {
    const parsed = await getContext() as { auth_commands: string[] };
  ...
  it("includes readonly_safe flag on each verb", async () => {
    const parsed = await getContext() as { verbs: { name: string; readonly_safe: boolean }[] };
    expect(parsed.verbs.find((v) => v.name === "get")!.readonly_safe).toBe(true);
    expect(parsed.verbs.find((v) => v.name === "create")!.readonly_safe).toBe(false);
  });
});
