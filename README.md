...
Fastest path — `frappe-ctl skills install` copies it to `<agent-dir>/frappe-ctl/SKILL.md` in every AI agent dir it detects in your project (`.claude/skills/`, `.cursor/skills/`, `.codex/skills/`, and 13 more), plus a common `.agents/skills/` path — the standard [Agent Skills](https://agentskills.io) directory shape these tools actually auto-discover. Run `frappe-ctl skills install --all` to install for every supported agent regardless of detection, or `--global` to install into your home directory instead of the current project.
